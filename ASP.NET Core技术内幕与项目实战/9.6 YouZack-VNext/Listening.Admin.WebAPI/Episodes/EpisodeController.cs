...
public class EpisodeController : ControllerBase
{
    ...
    public async Task<ActionResult<Guid>> Add(EpisodeAddRequest req)
    {
        ...
        else
        {
            ...
            EncodingEpisodeInfo encodingEpisode = new EncodingEpisodeInfo(episodeId, req.Name, req.AlbumId,
                req.DurationInSecond, req.Subtitle, req.SubtitleType, "Created");
            await _encodingEpisodeHelper.AddEncodingEpisodeAsync(episodeId, encodingEpisode);

            //通知转码
            _eventBus.Publish("MediaEncoding.Created",
                new { MediaId = episodeId, MediaUrl = req.AudioUrl, OutputFormat = "m4a", SourceSystem = "Listening" });//启动转码
            return episodeId;
        }
    }
