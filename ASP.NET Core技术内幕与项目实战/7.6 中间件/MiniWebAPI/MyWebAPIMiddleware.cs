 ...
 * 
 * 对于控制器类做如下简单的约定：
 *  1.控制器类的类名以Contriller结尾，除去结尾的Controller就是控制器的名字
 *  2.控制器类中所有的public方法就是动作方法，方法的名字就是动作方法的名字
 *  3.请求路径固定为“/Controller/Action”
 *  4.动作方法不支持重载，不支持通过【HttpGet】等方式绑定特点的Http谓词
 *  5.动作方法可以没有参数，如果有参数就只支持一个参数，动作方法的参数除非是HttpContext类型，否则请求报文体会按照json格式进行反序列化
 *  6.动作方法的返回值会被序列化为Json字符串，然后发送到响应报文中，不支持IActionResult等类型的返回值
 */
public class MyWebAPIMiddleware
{
    ...
    /// <param name="sp">在运行的时候才能拿到能用的IServiceProvider，所以IServiceProvider不通过构造函数注入</param>
    /// <returns></returns>
    public async Task InvokeAsync(HttpContext context, IServiceProvider sp)
    {
        ...
        //使用Controller的名字和Action的名字来加载动作方法对应的MethodInfo类型的对象
