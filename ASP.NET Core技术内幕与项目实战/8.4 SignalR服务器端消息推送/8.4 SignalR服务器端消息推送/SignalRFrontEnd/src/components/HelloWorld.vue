<template>
    <fieldset>
        ...
        <div>
            用户Id：<input type="password"  v-model="state.loginData.userId">
        </div>
        <div>
        ...
        setup() {
            ...
            const startConn = async function () {
                ...
                connection = new signalR.HubConnectionBuilder()
                    .withUrl('https://localhost:7080/Hubs/ChatRoomHub2', options)
            ...
            const loginClick = async function () {
                const resp = await axios.post('https://localhost:7080/Test1/Login',
                    state.loginData);
